...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const STORE_KEY = "bb_state_v6";

  // Caps (until we make this configurable in Settings tabs)
  const ENT_CAP = 75;
  ...
  // Is there at least one Wednesday strictly after today and on/before next pay date?
  ...
  /* ---------- Settings (balances + payday + entertainment cap) ---------- */
  $("btnSettings")?.addEventListener("click", () => {
    ...
    // Enforce entertainment hard cap by pushing overflow to savings
...
Will the budget last through till the next check: ${willLast ? "yes" : "no"}${reasonLine ? `\n${reasonLine}` : ""}`
  ...
  btnCheck?.addEventListener("click", () => {
    ...
    if (cat === "snacks") {
      ...
      // Entertainment rule: simple balance enforcement + cap awareness
      if (state.ent <= 0) {
        ok = false;
        remaining = state.ent;
        ...
        // Friendly note if at/over cap (does not affect buying)
        if (state.ent >= ENT_CAP) {
